		...
		<div class="view-op">
			<div class="row">
					...
					<div class="row">
						...
						<div class="col-sm-4">
							...
							<div class="input-group">
								<select class="form-control select2" data-placeholder="Search UID/Enter Mobile No." name="customer_id" id="customer_id" autofocus required>
				...
				<div class="section p-t-10">
					<form id="paymentInfo" method="POST" enctype='multipart/form-data' action="{{ route('customer-payments.store') }}" >
						@csrf
						<div class="row">							
							<div class="pay-box">
								<table id="" class="table pay">
									...
									<tbody>
										<tr>
											<input readonly name="user_email" id="user_email" type="hidden" class="form-control" 	value="">
											<td><input name="user_uid" id="user_uid" type="text" class="form-control" value=""></td>
											<!-- <td><input readonly name="user_position" id="user_position" type="text" class="form-control" value=""></td> -->
											<td><input readonly name="user_name" id="user_name" type="text" class="form-control" value=""></td>
											<td><input readonly name="user_mobile_no" id="user_mobile_no" type="text" class="form-control" value=""></td>
											<!-- <td><input readonly name="user_location" id="user_location" type="text" class="form-control" value=""></td> -->
											<td><input readonly name="user_credit_bal" id="user_credit_bal" type="text" class="form-control" value=""></td>
											<td><input readonly name="user_debit_bal" id="user_debit_bal" type="text" class="form-control" value=""></td>
											<td><input readonly name="user_subscription" id="user_subscription" type="text" class="form-control" value=""><span id="subscription-not-avail" class="hide" style="color:red;">No Plan Available</span></td>
											<td><input readonly name="user_validity" id="user_validity" type="text" class="form-control" value=""></td>
											<td><input readonly name="user_status" id="user_status" type="text" class="form-control" value=""></td>
										</tr>
									</tbody>
								</table>
								<!-- <div class="">
							...
							<div class="pay-wrap">
								<div class="row" id="fetch_results">
									<div class="col-2 text-center">
										...
										<div class="form-group" id="show_trip_ids">
										</div>
									...
									<div class="pay-section">
										...
										<div id="bank_info_div" class="pay-info">
											...
											<div id="cheque_div" class="hide">
												<div class="form-group col-3">
													<input id="user_payment_bank_cheque_name" type="text" class="form-control" name="user_payment_bank_cheque_name">
													<label>Name in Cheque.</label>
												</div>
												<div class="form-group col-3">
													<input id="user_payment_bank_cheque_no" type="text" class="form-control" name="user_payment_bank_cheque_no">
													<label>Cheque No.</label>
												</div>
												<div class="form-group col-3">
													<input id="user_payment_bank_account_no" type="text" class="form-control" name="user_payment_bank_account_no">
													<label>Account No.</label>
												</div>
												<div class="form-group col-3">
													<select id="user_bank_name" type="text" class="form-control select2" name="user_bank_name" data-placeholder="Select Bank Name">
													...
													<label id="user_bank_name-error" class="error m-0" for="user_bank_name"></label>
													<label>Select Bank</label>
												</div>
												<div class="form-group col-3">
													<input type="text" name="user_payment_bank_branch" id="user_bank_branch" class="form-control">
													<!-- <select id="user_bank_branch" type="text" class="form-control" name="user_payment_bank_branch">
										...
										<!-- <div class="" id="tranx_id_div"> -->
											...
											<!-- <div class="form-group">
												...
												<input type="hidden" name="user_payment_response[]" id="user_payment_response">
											</div> -->
					...
					<table id="payments" class="table pay-list table-bordered table-striped {{ count($payments) > 0 ? 'datatable' : '' }}" data-page-length="10">
						...
						<tbody>
							@if(!empty($customer_payments))
								...
								<tr>
									<td class="pay-col">#</td>
									<td class="pay-col-1">{{ $payment['user_cid'] ? $payment['user_cid'] : '-' }}</td>
									<td class="pay-col-2">{{ $payment['user_order_payment_purpose'] ? $payment['user_order_payment_purpose'] : '-' }}</td>
									<td class="pay-col-2">{{ $payment['user_order_pay_mode'] ? $payment['user_order_pay_mode'] : '-' }}</td>
									<td class="pay-col-2">{{ $payment['user_order_amount'] ? $payment['user_order_amount'] : '-' }}</td>
									<td class="pay-col-3">{{ $payment['created_at'] ? \Carbon\Carbon::parse($payment['created_at'])->format('d-m-Y H:i a') : '-' }} </td>
									<td class="pay-col-4">{{ $payment['user_order_status'] ? $payment['user_order_status'] : '-' }}</td>
									<td class="pay-col-5">
									<a href="{{ route('customer-payments.show', ['pay_id' => encrypt($payment['user_order_id'])]) }}"><button class="btn btn-xs btn btn-primary" type="button" data-toggle="tooltip" title="View!"><i class="fa fa-eye"></i></button></a>
									@if($payment['user_order_status'] == 'approved')
										<button class="btn btn-xs btn-success" type="button" data-toggle="tooltip" title="Approved!"><i class="fa fa-check"></i></button>
									@elseif($payment['user_order_status'] == 'received' || $payment['user_order_status'] == 'Success')
										<button class="btn btn-xs bg-orange" type="button" onclick="sendForApproval('{{ $payment['user_order_id'] }}')"><i class="fa fa-send-o" data-toggle="tooltip" title="Send for Approval"></i></button>
										<button class="btn btn-xs btn-danger" type="button" onclick="delete_payment('{{ $payment['user_order_id'] }}')"><i class="fa fa-trash" data-toggle="tooltip" title="Delete!"></i></button>
										@can('payment_edit')
										<a href="{{ route('customer-payments.edit',[ encrypt($payment['user_order_id']) ]) }}"><button class="btn btn-xs bg-olive"><i class="fa fa-edit"></i></button></a>
										@endcan
									@elseif($payment['user_order_status'] == 'pending' || $payment['user_order_status'] == 'Pending')
										<button class="btn btn-xs btn-received" type="button" onclick="mark_as_receive_payment('{{ $payment['user_order_id'] }}')"><i class="fa fa-arrow-right" data-toggle="tooltip" title="Mark As Received!"></i></button>
									@elseif($payment['user_order_status'] == 'waiting_for_approval')
									...
									<td>
									@if($payment['user_order_status'] == 'waiting_for_approval' && $payment['user_order_payment_is_approved'] == 0)
									@can('payment_approve')
									<a href="#" onclick="verifyByAdmin('{{ $payment['user_order_id'] }}')" data-toggle="tooltip" data-placement="top" title="click to approve payment">
										<i class="fa fa-share-square-o fa-aprv" aria-hidden="true"></i>
									</a>
									@endcan
									@elseif($payment['user_order_status'] == 'approved')
									...
									@endif</td>
									<!-- <td>@if($payment['user_order_status'] == 'approved')
									<a href="{{ route('payment-view', ['pay_id' => encrypt($payment['user_order_id'])]) }}"><button class="btn btn-xs btn" type="button" data-toggle="tooltip" title="View!"><i class="fa fa-undo"></i></button></a>
									@endif</td> -->
									<td><a href="{{ route('pdfview',['download'=> encrypt($payment['user_order_id'])]) }}"><i class="fa fa-print"></i></a></td>
									<td></td>
								</tr>
								@endforeach
							@else
...
<script type="text/javascript">
	...
	// {
	$("#customer_id-input").on("change", function() {
		// var user_uid = $(".select2-search__field").val();
		var user_uid = this.value;
			...
			$.ajax({
				...
				success : function(data){
					...
					if(!(result.length == 0)){
						...
						{
							html += '<option value="'+value.op_user_id+'">'+value.user_mobile_no +' ('+value.user_uid+') </option>';
						});
					}
					else{
	...
	$(document).ready(function() {
		...
		$(document.body).on("blur keypress","#user_uid",function(){
			$(':input','#fetch_results')
			...
			if(user_uid != ''){
				getPaymentDetails(user_uid, type ='by_uid');
			}
		});
		...
		{
			var op_pay_uid_status = $('#user_uid').val().trim();
		...
		$('#payment_purpose').change(function () {
			...
			if(payment_purpose == 'trip_payments'){
				...
				if(customer_id == null || customer_id == ''){
					...
					$.ajax({
						...
						success : function(data)
						{
							...
							$('#show_trip_ids').append(data);			
						}
					});
				}	
				
			}
			else if(payment_purpose == 'subscription'){
...
<script>
	...
	function set_customer_details(){
		var html7 = '';var result = '{{ $customer_details }}';
        var res = JSON.parse(result.replace(/&quot;/g,'"'));
        if(!(res.length == 0)){
            html7 += '<option value="">Select UID/Select Mobile No</option>';
            $.each(res, function (key, value)
            {
                html7 += '<option value="'+value.user_id+'">'+value.user_mobile_no +' ('+value.user_uid+') </option>';
            });
       }
       $('#customer_id').append(html7);
       initSelect();
	}
	...
	function mark_as_receive_payment(id)
	{
		...
		{
			$.ajax({
				url :"{{ route('markAsReceived-customer-payment') }}",
				method:"POST",
				data: {
					"_token": "{{ csrf_token() }}",
					"payment_id": id
	...
	function getFullResponse(payment_id)
	{
		$.ajax({
			...
			success : function(data)
			{
				$('#user_payment_response').val(data)
			}
		});
	}
	...
	function sendForApproval(order_id){
		...
		{
			$.ajax({
				url :"{{ route('send-for-approval-customer-payment') }}",
				method:"POST",
				data: {
					"_token": "{{ csrf_token() }}",
					"payment_id": order_id
	...
	function getPaymentDetails(user_uid, type){
		$.ajax({
			url :"{{ route('get-payment-details/customer') }}",
			method:"POST",
			data: {
				"_token": "{{ csrf_token() }}",
				"user_uid": user_uid,
			...
			success : function(data){
				...
				console.log(result);
				if(!(result == null)){
					$('#user_uid').val(result.user_uid);
					$('#user_id').val(result.user_id);
					$('#user_position').val();
					$('#user_name').val(result.user_first_name);
					$('#user_mobile_no').val(result.user_mobile_no);
					$('#user_email').val(result.email);
					$('#user_location').val();
					$('#user_credit_bal').val(result.credit_balance);
					$('#user_debit_bal').val(result.debit_balance);
					...
					// else{
					// 	$('#subscription-not-avail').addClass('hide');
					// 	$('#user_subscription').val(result.subscription.plan_name);
					// 	$('#user_validity').val(result.subscription.plan_validity);
					// 	$('#user_status').val(result.subscription.plan_status);
					// }
