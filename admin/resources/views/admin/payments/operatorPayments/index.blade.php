							...
							@if(!empty($payments))
								...
								<tr>
									...
									@elseif($payment['op_order_status'] == 'pending' || $payment['op_order_status'] == 'Pending')
										<button class="btn btn-xs btn-received" type="button" onclick="mark_as_receive_payment('{{ $payment['op_order_id'] }}')"><i class="fa fa-arrow-right" data-toggle="tooltip" title="Mark As Received!"></i></button>
			...
			$.ajax({
				...
				success : function(data)
				{
					...
					if(result.status == 'success'){
						...
						{ 
							window.location.href = "{{ url('/payments/operator') }}"
						});
					}
					else{
						swal({title: "Oops!", text: result.msg, type: result.status}).then(function()
